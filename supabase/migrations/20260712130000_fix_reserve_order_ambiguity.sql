-- Fix ambiguous subtotal_cents/total_cents in reserve_order RETURN QUERY
...
begin
  ...
  loop
    ...
    set subtotal_cents = o.subtotal_cents + item_total,
        total_cents = o.total_cents + item_total
    where o.id = order_record.id;
  end loop;

  return query
  select o.id, o.subtotal_cents, o.total_cents, o.reservation_expires_at
